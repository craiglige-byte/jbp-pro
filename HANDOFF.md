...
- **交付日期**: 2026-06-30
- **最后更新**: 2026-07-31
- **PRD 版本**: PRD-01 V1.6 / PRD-02 V1.11 / PRD-03 V2.6 / PRD-04 V2.15 / PRD-05 V2.1 / PRD-06 V1.8 / PRD-07 V2.8 / PRD-08 V1.3
- **开发服务器端口**: 5002（可能因端口占用自动调整）
...
### FY2027 财年全面更新 (PRD-03 V2.4-V2.5)
- 12 处硬编码「2026财年(2025.12-2026.11)」→「2027财年(2026.12-2027.11)」
- 默认 period: FY2025 → FY2027
- 文案优化：副标题「本年度」→「下一财年」；授权地图「本财年」→「下一财年」

### 盈利拆解表头 (PRD-04 V2.14)
- 年度利润规划表列头：绝对年份 → 相对标签「今年实际/今年占比/明年目标/明年占比」
- 此后跨财年无需手动更新表头

### 部署架构修复
- 修复 GitHub Pages 推送目标：gh-pages 分支 → main 分支 docs/ 目录（Pages 配置 root）
- 修复 asset 路径：/assets/ → ./assets/（相对路径兼容 GitHub Pages 子目录）
- vite.config.ts 添加 `base: '/jbp-pro/'` 确保构建路径正确

### 仓库整理
- 删除 docs/assets 中 17 个旧版构建产物，仅保留最新 2 个文件
- 全部 6 个 HTML 入口页统一指向同一份 SPA bundle
- 源代码完整提交至 main 分支（App.tsx + 20 组件 + 配置 + PRD）
- .env → .env.example 模板化，.env 加入 .gitignore
- 删除临时的 dist/ 和乱码 merge 脚本

## 最近变更摘要 (2026-07-24)

### 经营回顾 — 运营资源投入 (PRD-02a)
- **车辆**: 标题「车辆配置」→「车辆数量」，细项从 3 类车型 → 单一项「元气车辆数量」
- **人员**: 从 4 项 → 5 项（客户团队人数/业务人员/管理人员/文职人员/后勤人员）
- **资金**: 移除细分项展示（常态库存资金/市场垫资/信贷保证金），只显示总额
- **数据迁移**: localStorage 恢复时强制覆盖 vehicles/personnel/capital 为最新结构

### 经营回顾 — 经营数据概览 (PRD-02a)
- 「人年均销售额」→「vpo」，单位「万/人/年」→「元/店」
...
**客户分级金字塔**:
- 去掉中文标题（战略客户/核心客户…），只保留 Class S/A/B/C 标徽
- 去掉 criteria 描述（年销>30万…）
- 去掉利润贡献，只保留销量贡献
- 客户明细表：移除利润列，只保留客户名称+销售额；标题改为 Class X 格式
...
**团队业绩分析**:
- 移除利润率列；新增「岗位」列
- `JBPTeamAnalysis` 类型新增 `role: string` 字段

### 授权经营区域 (PRD-02)
- 录入数据弹窗：表头「达成率」→「业绩达成率」

### 类型定义
- `types.ts`: `JBPTeamAnalysis` 新增 `role: string`

## 最近变更摘要 (2026-07-31)

### 进货拆解弹窗 (PRD-04 V2.15)
- **去向导化重构**: PurchaseBreakdownWizard 大幅精简（-1333行），从 Wizard 步骤式改为直接月份填写
- **三项优化**: 弹窗改名、输入框可视区分（蓝色边框）、按钮校验逻辑
- **样式微调**: 场景列只读、输入框统一蓝色、按钮文案内联

### 设定目标大改 (PRD-03 V2.6)
- **目标重排序**: 实现销售目标移至首位，达成进货承诺移至第二位；OBJECTIVE_TEMPLATES 同步
- **销售目标单位改为万元**: 输入万元值，括号约合箱=floor(万×10000/45)；验证同步更新
- **达成进货承诺去公式化**: 完全自由填写，去除中文大写转换、去除 onFocus 清空、去除自动联动计算销售目标
- localStorage 迁移自动重排旧顺序

### 经营回顾 — 市场潜力与竞品分析 (PRD-02 V1.11)
- 卡片新增「区域今年分销额」（万元），默认 1000，纯展示不入录入弹窗
- 位置：人均饮用量下方，cyan 配色 + TrendingUp 图标
- `marketStats` 类型新增 `regionalDistributionAmount: string`
- localStorage 迁移自动补齐新字段

## 最近变更摘要 (2026-07-27)

### 规划预算大改 (PRD-06 V1.8)
- **仓库租赁**: 配置方式 长租/短租/自有 → 租赁/自有
- **车辆配置**: 车型 select → 文本输入
- **人员架构 V2.0**: 删除人数/月薪/社保等 6 列；年总成本→手动输入；新增自有业代；元气分摊 2 位小数
- **费用规划 V2.0**: 列重排；总投入预算→手动输入；经销商承担→自动计算（经销商%=100%-厂家%，金额=总投入×占比，费用占比=该行/总计）
- **资金准备**: 新增冰柜押金枚举

### 落实行动大改 (PRD-05 V2.1)
- 负责人弹窗：w-56→w-72，left-0→right-0，按钮加 whitespace-nowrap
- 排期弹窗：w-48→w-64，left-0→right-0，输入框和按钮尺寸统一

### 经营回顾 (PRD-02 V1.10)
- 运营资源投入：仓库面积/资金投入下方新增元气品牌分摊行，格式对齐车辆明细

### 预览计划 (PRD-07)
- 人员架构/费用规划弹窗列表与 BudgetStep 对齐；人员汇总卡从人数改为总成本

## 最近变更摘要 (2026-07-24 下午)

### 落实行动 (PRD-05)
- **负责人与排期交互统一重构**: 弹窗加宽防止按钮溢出，输入框旁增加「添加」按钮
- **排期交互**: 改为直接多选 Q1/Q2/Q3/Q4 + 自定义输入添加，替代旧版下拉方式

### 拆解策略 (PRD-04)
- **销售目标拆解**: Step1 场景列改为只读（自动匹配当前版本，不可手动编辑）
- **总进货列**: 统一改为 4 位小数显示
- **品类金额**: 统一改为 4 位小数显示
- **季度品类 (Step2)**: 移除品类特点中的季度占比建议；改为只填百分比，金额自动计算只读

### 规划预算 (PRD-06)
- 进货规划表万元统一显示
- 授权地图「未确认」按钮置灰（需先确认授权范围）
