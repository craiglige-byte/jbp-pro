# JBP Pro 交付说明

## 版本信息

- **项目名称**: JBP Pro — 联合生意规划助手
- **交付日期**: 2026-06-30
- **最后更新**: 2026-08-14
- **PRD 版本**: PRD-01 V1.6 / PRD-02 V1.11 / PRD-03 V2.7 / PRD-04 V2.20 / PRD-05 V2.2 / PRD-06 V2.4 / PRD-07 V2.11 / PRD-08 V1.3
- **开发服务器端口**: 5000（可能因端口占用自动调整）

## 技术栈

| 技术 | 版本 |
|------|------|
| React | 18.2.0 |
| Vite | 6.4.1 |
| TypeScript | 5.8 |
| Tailwind CSS | 4.2.1 |
| Recharts | 2.12.7 |
| Google Maps JS API | 1.16.2 |
| Google GenAI | 1.38.0 |

## 快速启动

### 1. 安装依赖

```bash
npm install
```

### 2. 配置 API Key（可选）

复制 `.env` 文件，将其中的 `GEMINI_API_KEY` 替换为真实的 Google Gemini API Key。
不配置 API Key 时，AI 相关功能不可用，其他功能正常。

### 3. 启动开发服务器

```bash
npm run dev
```

### 4. 访问应用

- **大版本 (≥500万)**: `http://localhost:5002/`
- **小版本 (＜500万)**: `http://localhost:5002/?version=small`

### 5. 构建生产版本

```bash
npm run build
```

构建产物在 `dist/` 目录下。

## 双版本

本项目支持两种业务规模，通过 URL 参数切换：

| 版本 | URL | PlanVersion | 说明 |
|------|-----|-------------|------|
| ≥500万 (大版) | `/` | `large` | 完整 7 步流程，含策略、行动战术 |
| ＜500万 (小版) | `/?version=small` | `small` | 简化流程，无策略输入，拆解完成即可下一步 |

独立 HTML 文件位于 `dist/`：
- `JBP_Pro_大版本_大于等于500万.html` — 大版本独立页面
- `JBP_Pro_小版本_小于500万.html` — 小版本独立页面（通过 `window.JBP_PLAN_VERSION="small"` 设置）

## 项目结构

```
jbp2027/
├── App.tsx                     # 主应用入口，版本检测
├── index.tsx                   # React 渲染入口
├── index.html                  # Vite HTML 模板
├── index.css                   # 全局样式 / Tailwind
├── constants.ts                # 常量 / 模板数据
├── types.ts                    # TypeScript 类型定义
├── demoData.ts                 # 演示参考案例（完整 7 步数据）
├── vite.config.ts              # Vite 构建配置
├── package.json                # 依赖与脚本
├── tsconfig.json               # TypeScript 配置
├── .env                        # 环境变量模板
├── README.md                   # 简要说明
├── HANDOFF.md                  # 本文件
│
├── components/
│   ├── InfoStep.tsx            # Step 1: 基本信息
│   ├── BusinessReviewStep.tsx  # Step 2: 经营回顾
│   ├── ObjectiveStep.tsx       # Step 3: 设定目标
│   ├── StrategyStep.tsx        # Step 4: 拆解策略
│   ├── ActionStep.tsx          # Step 5: 落实行动
│   ├── BudgetStep.tsx          # Step 6: 规划预算
│   ├── ReviewStep.tsx          # Step 7: 预览计划
│   ├── StepWizard.tsx          # 通用向导框架
│   ├── CalendarStep.tsx        # 日历组件
│   ├── PurchaseBreakdown.tsx   # 进货拆解
│   ├── PurchaseBreakdownWizard.tsx
│   ├── SalesBreakdown.tsx      # 销售拆解
│   ├── SalesBreakdownWizard.tsx
│   ├── InventoryBreakdown.tsx  # 库存拆解
│   ├── InventoryBreakdownWizard.tsx
│   ├── ProfitabilityBreakdown.tsx # 盈利拆解
│   └── ProfitabilityBreakdownWizard.tsx
│
├── docs/prd/                   # 产品需求文档
│   ├── README.md               # PRD 索引
│   ├── PRD-01-基本信息.md
│   ├── PRD-02-经营回顾.md
│   ├── PRD-02a-商维度.md
│   ├── PRD-02b-终端维度.md
│   ├── PRD-03-设定目标.md
│   ├── PRD-04-拆解策略.md      # V2.19
│   ├── PRD-05-落实行动.md       # V2.1
│   ├── PRD-06-规划预算.md       # V2.3
│   ├── PRD-07-预览计划.md       # V2.9
│   └── PRD-08-审批页面.md       # V1.3
│
├── dist/                       # Vite 构建输出（gitignore）
│   ├── index.html
│   └── assets/
│
├── docs/                       # GitHub Pages 部署
│   ├── index.html              # 默认入口
│   ├── large.html              # ≥500万 SPA 入口
│   ├── small.html              # ＜500万 SPA 入口
│   ├── JBP_Pro_大版本_大于等于500万.html
│   ├── JBP_Pro_小版本_小于500万.html
│   ├── JBP_审批_大版本.html
│   ├── JBP_审批_小版本.html
│   └── assets/
│
└── public/                     # 静态资源（已清理）
```

## 最近变更摘要 (2026-06-30)

### 全局框架
- **V1.1**: 新增演示参考案例 `demoData.ts`，首次打开自动加载完整示例
- **V1.1**: URL 路径版控 `/large` / `/small`，页面标题动态显示版本
- **V1.2**: 步骤条 + Header 置顶固定，长页面滚动不丢失导航

### 经营回顾 (PRD-02)
- **V1.6**: BugFix — 「新地图范围」tab 中 Math.random() 导致数据抖动，改为确定性计算

### 拆解策略 (PRD-04)
- **V2.1**: 库存拆解 — 销售占比(%)、目标周转(天)、下月销预估(箱) 输入限制非负两位小数
- **V2.2**: 盈利拆解 — 去年实际/今年目标/今年目标(万) 统一小数输入校验
- **V2.3**: 库存拆解 — 品类占比总和必须 = 100%，否则下一步禁用
- **V2.4**: 小版(＜500万) — 跳过策略校验，拆解完成即可下一步

### 规划预算 (PRD-06)
- **V1.1**: 5 子模块数字字段统一输入校验规范
- **V1.2**: 仓库租赁/人员架构 — 预算上限/预算对比列对齐修复
- **V1.3**: 四个模块预算上限新增来源说明文案
- **V1.4**: 车辆配置 — X 改 Trash2、允许删至 0 行

### BugFix
- 预览计划 JS 报错 `expandedActionId is not defined` 修复
- 经营回顾「新地图范围」Math.random() 数据抖动修复
- 库存拆解按钮文案修正

## 输入校验规范

所有数字输入字段统一使用以下规则：

1. `type="text"` + `inputMode="decimal"` 取代 `type="number"`
2. 正则过滤非数字/非小数点字符
3. 仅保留第一个小数点
4. 小数位截断至 2 位（不四舍五入）
5. 空值 / 仅小数点 → 转为 0

涉及文件：`InventoryBreakdownWizard.tsx`、`ProfitabilityBreakdownWizard.tsx`、`BudgetStep.tsx`

## 最近变更摘要 (2026-07-06)

### 🔧 BugFix：年份逻辑 + 类型修复
- **year 2027**: 5 文件 8 处新增 `period.includes('2027')`
- **dailyCapacity**: `JBPVehicleBudget` 类型补充 `dailyCapacity: number`，demoData 恢复值

### 📐 设定目标 (PRD-03 V2.2)
- 销售目标联动公式：`箱数 = floor(进货金额/42)`, `金额 = 箱数 × 56`（符合 PRD 100→2箱112元）

### 🔧 BugFix：年份逻辑 (原始)
- **5 文件 8 处**新增 `period.includes('2027')`：StrategyStep(5)、ReviewStep(1)、ObjectiveStep(1)、CalendarStep(1)
- 根因：period="2027 FY" 时月份生成回退到 2023 年，导致所有拆解表数据不匹配

### 📊 库存拆解 (PRD-04 V2.5-V2.11)
- Step2「下月销预估」→「销量预估」只读，同月匹配 salesPlan.thisYearTarget
- useEffect + 初始化强制刷新，弹窗/汇总表双联动
- 表头说明文案、品类占比行、单位标注（箱）、0 值显示修复

### 📦 进货/销售拆解 (PRD-04 V2.7-V2.8)
- 进货规划表：「今年计划」→「明年计划」，金额加「元」
- 销售规划表：各区+全司合计加「箱」，填充完整 monthlyAreaTargets 数据

### 💰 盈利拆解 (PRD-04 V2.12)
- Step1「今年实际」只读，全字段 filterDecimal

### 📋 规划预算 (PRD-06 V1.5-V1.6)
- 仓库/车辆预算上限+对比加「元」；人员/费用加「万」保留两位小数；资金加「元」

### 🗺️ 基本信息 (PRD-01 V1.5)
- 地图双按钮：刷新 + 查看详情/编辑

### 📝 经营回顾 (PRD-02 V1.7)
- 利润详情弹窗输入框：字符串存储 + filterDecimal

### ✅ 预览计划 (PRD-07 V2.1-V2.2)
- **V2.1**: 归档审批「发生变更」→「授权区域发生变更，请重新编辑。」
- **V2.2**: 弹窗标题→「年度计划归档提交」，每个经销商双版本分行展示（≥500万/＜500万），合并名称列，独立多选（Set），自动选中唯一正常版本，弹窗加宽 max-w-4xl，增加「未生成年度计划」状态

### 🎯 参考案例
- demoData.ts 替换为小黄鸭商贸 Coze 完整数据（4目标+5模块预算+12月趋势）
- 独立 HTML 开启 JBP_READONLY 只读预览模式
- dist/ 全量重建，旧文件清理

## 最近变更摘要 (2026-07-08)

### 全局 UI
- 步骤条+头部压缩（py-2/py-1.5，图标缩小），合并为 sticky 容器固定定位
- 规划预算所有必填字段表头加红色 *

### 基本信息 (PRD-01 V1.6)
- 「查看详情/编辑」→「查看/调整授权区域」

### 经营回顾 (PRD-02 V1.8)
- 新增「授权经营区域」模块（经营品类分析和客户分级分析之间），含地图+左侧情报面板+右上角录入数据/刷新/调整授权区域
- 录入数据面板：市场指标单位标签外置（万/亿/升·年），filterDecimal；竞品表增加品牌列、业绩单位万、网点数单位家、达成率0-100%
- 授权地图范围确认移到 Step3 顶部，校验也移到 Step3

### 设定目标 (PRD-03 V2.3)
- 销售目标公式修正：`箱=floor(进货金额/42)`, `元=箱×56`

### 拆解策略 (PRD-04 V2.14)
- JBPVehicleBudget 类型加回 dailyCapacity

### 规划预算 (PRD-06 V1.7)
- 所有必填字段表头加红色 *

### 预览计划 (PRD-07 V2.4)
- 归档弹窗简化：单行显示，仅两种状态（可提交/授权区域发生变更），去掉版本选择，5经销商
- 新增刷新按钮，文案优化

### 审批页面 (PRD-08 V1.0)
- 新增只读审批页面：60秒倒计时+通过/驳回按钮
- GitHub Pages 部署上线：https://craiglige-byte.github.io/jbp-pro/

### 基础设施
- 去掉了底部 footer
- 参考案例链接从 localhost:10000 改为 GitHub Pages 固定链接
- dist/ → docs/ 适配 GitHub Pages 部署

## 最近变更摘要 (2026-07-20)

### FY2027 财年全面更新 (PRD-03 V2.4-V2.5)
- 12 处硬编码「2026财年(2025.12-2026.11)」→「2027财年(2026.12-2027.11)」
- 默认 period: FY2025 → FY2027
- 文案优化：副标题「本年度」→「下一财年」；授权地图「本财年」→「下一财年」

### 盈利拆解表头 (PRD-04 V2.14)
- 年度利润规划表列头：绝对年份 → 相对标签「今年实际/今年占比/明年目标/明年占比」
- 此后跨财年无需手动更新表头

### 部署架构修复
- 修复 GitHub Pages 推送目标：gh-pages 分支 → main 分支 docs/ 目录（Pages 配置 root）
- 修复 asset 路径：/assets/ → ./assets/（相对路径兼容 GitHub Pages 子目录）
- vite.config.ts 添加 `base: '/jbp-pro/'` 确保构建路径正确

### 仓库整理
- 删除 docs/assets 中 17 个旧版构建产物，仅保留最新 2 个文件
- 全部 6 个 HTML 入口页统一指向同一份 SPA bundle
- 源代码完整提交至 main 分支（App.tsx + 20 组件 + 配置 + PRD）
- .env → .env.example 模板化，.env 加入 .gitignore
- 删除临时的 dist/ 和乱码 merge 脚本

## 最近变更摘要 (2026-07-24)

### 经营回顾 — 运营资源投入 (PRD-02a)
- **车辆**: 标题「车辆配置」→「车辆数量」，细项从 3 类车型 → 单一项「元气车辆数量」
- **人员**: 从 4 项 → 5 项（客户团队人数/业务人员/管理人员/文职人员/后勤人员）
- **资金**: 移除细分项展示（常态库存资金/市场垫资/信贷保证金），只显示总额
- **数据迁移**: localStorage 恢复时强制覆盖 vehicles/personnel/capital 为最新结构

### 经营回顾 — 经营数据概览 (PRD-02a)
- 「人年均销售额」→「vpo」，单位「万/人/年」→「元/店」

### 经营回顾 — 经营趋势分析 (PRD-02a)
- 移除利润率列（表格）+ 利润率曲线（图表）

### 经营回顾 — 经营品类分析 (PRD-02a)
- 移除利润率列，表格「品类/销售额」(2列)

### 经营回顾 — 终端维度 (PRD-02b)

**客户分级金字塔**:
- 去掉中文标题（战略客户/核心客户…），只保留 Class S/A/B/C 标徽
- 去掉 criteria 描述（年销>30万…）
- 去掉利润贡献，只保留销量贡献
- 客户明细表：移除利润列，只保留客户名称+销售额；标题改为 Class X 格式

**渠道表现分析**:
- 移除利润率列（表格）+ 利润率折线（图表）

**团队业绩分析**:
- 移除利润率列；新增「岗位」列
- `JBPTeamAnalysis` 类型新增 `role: string` 字段

### 授权经营区域 (PRD-02)
- 录入数据弹窗：表头「达成率」→「业绩达成率」

### 类型定义
- `types.ts`: `JBPTeamAnalysis` 新增 `role: string`

## 最近变更摘要 (2026-07-31)

### 进货拆解弹窗 (PRD-04 V2.15)
- **去向导化重构**: PurchaseBreakdownWizard 大幅精简（-1333行），从 Wizard 步骤式改为直接月份填写
- **三项优化**: 弹窗改名、输入框可视区分（蓝色边框）、按钮校验逻辑
- **样式微调**: 场景列只读、输入框统一蓝色、按钮文案内联

### 设定目标大改 (PRD-03 V2.6)
- **目标重排序**: 实现销售目标移至首位，达成进货承诺移至第二位；OBJECTIVE_TEMPLATES 同步
- **销售目标单位改为万元**: 输入万元值，括号约合箱=floor(万×10000/45)；验证同步更新
- **达成进货承诺去公式化**: 完全自由填写，去除中文大写转换、去除 onFocus 清空、去除自动联动计算销售目标
- localStorage 迁移自动重排旧顺序

### 经营回顾 — 市场潜力与竞品分析 (PRD-02 V1.11)
- 卡片新增「区域今年分销额」（万元），默认 1000，纯展示不入录入弹窗
- 位置：人均饮用量下方，cyan 配色 + TrendingUp 图标
- `marketStats` 类型新增 `regionalDistributionAmount: string`
- localStorage 迁移自动补齐新字段

### 设定目标范围校验 (PRD-03 V2.7)
- **四模块实时校验**：销售目标（区域分销额×1.05~1.30）、进货承诺（销售目标×0.95~1.05）、库存健康（25~45天）、盈利能力（3%~12%）
- 低于/高于分开展示不同文案，红色提示置于模块下方，范围内自动消失

### 拆解策略模块排序 (PRD-04 V2.16)
- 固定进货在前、销售在后，不受设定目标数据顺序影响（displayObjectives sort）

### 规划预算四模块校验与改动 (PRD-06 V2.2)
- **仓库租赁**：元气专用面积50~2000㎡校验，下一步拦截
- **车辆配置**：车型改为单选（微型面包车/中型面包车/4.2米箱货）；进货承诺/车辆数∈[150~250]万校验，不拦截红字
- **人员架构**：新增「人数」列（非负整数）；专职业代+厂家业代80~200万/人校验；司机≥车辆数校验
- **资金准备**：厂家预付款≥进货承诺×25%；启动资金≥进货承诺×30%；分别命中提示

### 预览计划警告同步 (PRD-07 V2.9)
- 设定目标校验警告带到预览计划对应目标标题旁
- 规划预算校验警告带到预览计划对应预算卡片预算对比下方

## 最近变更摘要 (2026-08-14)

### 品类统一 (PRD-04 V2.20)
- 品类统一为 7 个：气泡水/电解质水/冰茶/维生素水/好自在/本榨/其他（新增「本榨」，占比合计 100%）
- 修复 demoData 销售/进货 plan 字段错位（实现销售目标 ↔ salesPlan、达成进货承诺 ↔ purchasePlan）

### 人员架构人数列 (PRD-06 V2.4 / PRD-07 V2.11)
- 预览计划人员架构详情弹窗补「人数」列，与规划预算页对齐

## 最近变更摘要 (2026-08-03)

### 基础设施
- **构建后脚本**: `npm run build` 自动复制 dist→docs + 更新全部 7 个 HTML 的 JS/CSS 引用

### 进货拆解弹窗 (PRD-04 V2.17)
- **输入交互修复**: 支持小数点 4 位小数输入，raw display 追踪不再被 toWanInput 覆盖
- **右上角文案**: 达成进货承诺显示「进货目标xx万元」替代长文案

### 仓库租赁比例校验 (PRD-06 V2.3)
- 每 100 万进货需 ≥50㎡ 元气专用面积，不足时红字提示；预算计划+预览同步

### 目标一致性校验 (PRD-04 V2.17)
- 拆解策略/规划预算下一步拦截：进货/销售/库存/利润目标变更后需重新拆解
- **BugFix (08-03)**: `validateTargetConsistency` 库存校验属性名修正 `monthlyInventory` → `monthlyPlan`
- **BugFix (08-03)**: 利润一致性校验改用 `objectiveTargetMargin`，不再比较计算利润率与目标利润率（StrategyStep + BudgetStep 两份代码同步修复）

### 库存拆解弹窗 (PRD-04 V2.18)
- 头部展示「总销售箱数 | 目标周转≤X天」
- 加权平均周转超目标则禁用下一步

### 盈利拆解弹窗 (PRD-04 V2.19)
- Step1 测算运营费用后校验净利润率 ≥ 目标利润率，否则禁用下一步

### 规划预算 — 人员架构校验 (PRD-06 V2.3)
- **BugFix (08-03)**: `purchaseAmountWan` 提取兼容旧格式（`¥xxx`、`（xxx万元）`）
- **BugFix (08-03)**: 专职业代+厂家业代个数为 0 时改为红字提示，不再静默跳过
- 校验公式: 进货承诺 / (专职业代+厂家业代) ∈ [80, 200]万

### 类型定义
- `JBPProfitabilityPlan` 新增 `objectiveTargetMargin?: number` 字段

## 最近变更摘要 (2026-07-27)

### 规划预算大改 (PRD-06 V1.8)
- **仓库租赁**: 配置方式 长租/短租/自有 → 租赁/自有
- **车辆配置**: 车型 select → 文本输入
- **人员架构 V2.0**: 删除人数/月薪/社保等 6 列；年总成本→手动输入；新增自有业代；元气分摊 2 位小数
- **费用规划 V2.0**: 列重排；总投入预算→手动输入；经销商承担→自动计算（经销商%=100%-厂家%，金额=总投入×占比，费用占比=该行/总计）
- **资金准备**: 新增冰柜押金枚举

### 落实行动大改 (PRD-05 V2.1)
- 负责人弹窗：w-56→w-72，left-0→right-0，按钮加 whitespace-nowrap
- 排期弹窗：w-48→w-64，left-0→right-0，输入框和按钮尺寸统一

### 经营回顾 (PRD-02 V1.10)
- 运营资源投入：仓库面积/资金投入下方新增元气品牌分摊行，格式对齐车辆明细

### 预览计划 (PRD-07)
- 人员架构/费用规划弹窗列表与 BudgetStep 对齐；人员汇总卡从人数改为总成本

## 最近变更摘要 (2026-07-24 下午)

### 落实行动 (PRD-05)
- **负责人与排期交互统一重构**: 弹窗加宽防止按钮溢出，输入框旁增加「添加」按钮
- **排期交互**: 改为直接多选 Q1/Q2/Q3/Q4 + 自定义输入添加，替代旧版下拉方式

### 拆解策略 (PRD-04)
- **销售目标拆解**: Step1 场景列改为只读（自动匹配当前版本，不可手动编辑）
- **总进货列**: 统一改为 4 位小数显示
- **品类金额**: 统一改为 4 位小数显示
- **季度品类 (Step2)**: 移除品类特点中的季度占比建议；改为只填百分比，金额自动计算只读

### 规划预算 (PRD-06)
- 进货规划表万元统一显示
- 授权地图「未确认」按钮置灰（需先确认授权范围）
