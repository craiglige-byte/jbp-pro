...
| 2026-08-03 | 仓库租赁新增比例校验：每100万进货需≥50㎡元气专用面积（0.5㎡/万），不满足时预算对比下方红字提示「元气专用面积不足」。预览计划同步展示 | V2.3 |
| 2026-08-04 | BugFix：人员架构校验——`purchaseAmountWan` 提取兼容旧格式（`¥xxx`、`（xxx万元）`），旧格式返回0导致整个校验块被跳过；专职业代+厂家业代个数为0时改为红字提示「请规划专职业代与厂家业代个数」，不再静默跳过 | V2.3 |
| 2026-07-31 | 四模块新增校验与改动：(1)仓库租赁：元气专用面积50~2000㎡校验，下一步拦截；(2)车辆配置：车型改为单选（微型面包车/中型面包车/4.2米箱货），进货承诺/车辆数∈[150~250]万校验，不拦截红字警告；(3)人员架构：新增「人数」列（非负整数），专职业代+厂家业代80~200万/人校验，司机≥车辆数校验；(4)资金准备：厂家预付款≥进货承诺×25%，启动资金≥进货承诺×30%，分别命中提示。预览计划同步展示警告 | V2.2 |
