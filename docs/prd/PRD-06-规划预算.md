...
| F03 | 人员架构 | 岗位/人数/年总成本(万元,直接输入)/品牌占比/元气分摊(2位小数) |
...
| 2026-06-30 | 新增 §6 输入校验规范：5 子模块 15 数字字段统一 R01-R05 规则 | V1.1 |
| 2026-06-30 | 仓库租赁/人员架构：预算上限/预算对比列对齐修正 | V1.2 |
| 2026-06-30 | 四个模块预算上限数字右侧新增来源说明 | V1.3 |
| 2026-06-30 | 车辆配置：Trash2 替换 X；允许删至 0 行 | V1.4 |
| 2026-07-03 | 仓库租赁/车辆配置：预算上限+对比加「元」单位 | V1.5 |
| 2026-07-03 | 人员架构/费用规划保留两位小数加「万」；资金准备加「元」 | V1.6 |
| 2026-07-08 | 所有必填字段表头加红色 * | V1.7 |
| 2026-07-27 | 仓库租赁：配置方式枚举改为「租赁/自有」；车辆配置：车型从 select 改为文本输入 | V1.8 |
| 2026-07-27 | 资金准备：新增「冰柜押金」枚举 | V1.9 |
| 2026-07-27 | 预览计划弹窗同步更新：人员架构/费用规划列表与 BudgetStep 对齐 | V1.10 |
| 2026-07-27 | 人员架构 V2.0：年总成本改为手动输入，删除拆分列（月薪/社保等6列），新增「自有业代」，元气分摊2位小数 | V2.0 |
| 2026-07-27 | 费用规划 V2.0：列重排，总投入预算改为输入，经销商承担改自动计算 | V2.1 |
| 2026-07-31 | 四模块新增校验与改动：(1)仓库50~2000㎡；(2)车型改单选+进货/车辆数[150,250]万；(3)人数列+专职业代80~200万/人+司机≥车辆；(4)资金预付款/启动资金校验。预览同步 | V2.2 |
| 2026-08-03 | 仓库租赁新增比例校验：每100万进货需≥50㎡元气专用面积；红字提示；预览同步 | V2.3 |
| 2026-08-04 | BugFix：人员架构 `purchaseAmountWan` 兼容旧格式；专职业代个数为0时红字提示 | V2.3 |
| 2026-08-14 | 人员架构人数列恢复显示（预览计划人员架构详情弹窗同步补人数列）| V2.4 |
