...
| 2026-07-08 | 新增「授权经营区域」模块（品类分析和客户分级之间）：地图+情报面板+刷新+录入数据；市场指标单位外置、filterDecimal；竞品表品牌/业绩万/网点家/达成率%；移除调整授权区域→提示小字 | V1.8 |
| 2026-07-15 | 网点数输入从两位小数改为非负整数（`handleDecimalInput 0`） | V1.9 |
