...
**默认品类**：电解质水（蓝#3b82f6, 450万, +18%）、气泡水（绿#10b981, 320万, +12%）、冰茶（橙#f59e0b, 280万, +8%）、维生素水（紫#8b5cf6, 180万, +25%）、好自在（粉#ec4899, 120万, +35%）、其他（灰#9ca3af, 50万, +5%）
...
| 2026-06-22 | F06/F07/F08：增加「今年地图范围」/「新地图范围」双 Tab；移除增长率列；新地图范围增加百分比差异标签 | V1.3 |
| 2026-06-22 | V1.5 授权地图范围确认移至经营回顾末尾 | V1.4 |
| 2026-06-26 | 客户分级从3级(A/B/C)升级为6级(S/A/B/C/其他/空)，3列×2行卡片布局；客户明细表移除增长率列；下一步校验新增授权地图范围确认检查 | V2.0 |
| 2026-06-30 | 经营数据概览添加利润额输入框 | V1.5 |
| 2026-06-30 | BugFix — 「新地图范围」tab 中 Math.random() 导致数据抖动，改为确定性计算（三处同修） | V1.6 |
| 2026-07-06 | 利润详情弹窗：所有输入框改为字符串存储 + `filterDecimal`，支持非负两位小数 | V1.7 |
| 2026-07-08 | 新增「授权经营区域」模块（品类分析和客户分级之间）：地图+情报面板+刷新+录入数据；市场指标单位外置、filterDecimal；竞品表品牌/业绩万/网点家/达成率% | V1.8 |
| 2026-07-15 | 网点数输入从两位小数改为非负整数 | V1.9 |
| 2026-07-27 | F01 运营资源投入：仓库面积/资金投入下方新增元气品牌分摊行，品牌占比取自详细预算 | V1.10 |
